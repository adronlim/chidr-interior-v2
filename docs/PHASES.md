...
| # | Step | Owner | Done? |
| --- | --- | --- | --- |
| 1 | Create Sanity project (`CHIDR Interior`, dataset `production`, visibility `public`) | User | ✓ (project id `abc12def` — dummy; real value in `apps/studio/.env`) |
...
| When | Event | Outcome |
| --- | --- | --- |
| 2026-05-29 | First `yarn studio:seed` run | **401 Unauthorized** — `projectUserNotFoundError`. Token's user (id shape: `g-XXXXXXXXXX`) was not a member of project `abc12def`. (IDs are dummies; reals in `apps/studio/.env`.) |
...
| 2026-05-29 | Env file cleanup | Removed misplaced `VITE_SANITY_PROJECT_ID=<token-shaped value>` from `apps/studio/.env`; created `apps/web/.env.local` with the correct `VITE_*` vars (project id `abc12def` — dummy; real lives in `.env`). |
...
1. Sign in to sanity.io with the account that owns CHIDR Interior (the one where project `abc12def` — dummy id — is visible at sanity.io/manage)
