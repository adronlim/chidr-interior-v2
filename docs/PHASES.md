...
| # | Step | Owner | Done? |
| --- | --- | --- | --- |
| 1 | Create Sanity project (`CHIDR Interior`, dataset `production`, visibility `public`) | User | ☐ |
| 2 | Copy project ID into `apps/studio/.env` (`SANITY_STUDIO_PROJECT_ID`) | User | ☐ |
| 3 | Copy project ID into `apps/web/.env.local` (`VITE_SANITY_PROJECT_ID`) | User | ☐ |
| 4 | Add CORS origins for `http://localhost:5173` and `http://localhost:3333` | User | ☐ |
| 5 | Generate Editor-scoped token at sanity.io → API → Tokens → paste into `SANITY_AUTH_TOKEN` | User | ☐ |
| 6 | `yarn studio:seed` — creates 9 projects + 3 categories + company + hero | User | ☐ |
...
### Blockers
- Sanity account creation requires the user's credentials — cannot be automated
